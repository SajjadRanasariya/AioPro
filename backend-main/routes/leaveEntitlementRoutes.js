...
import { getAll,getOne,addOne,updateOne,deleteOne } from "../controllers/leaveEntitlementController";
const router = Router();
...
router.post(
    ...
    async (req, res) => {
        try {
            ...
            res.send({ result: result, message: "Data added successfully" });
        } catch (error) {
            console.log(error);
            return res.status(500).json({ error: error.toString() });
        }
...
router.delete("/:id", passport.authenticate("jwt", { session: false }), async (req, res) => {
    try {
        ...
        res.send({ result: result, message: "Data deleted successfully" });
    } catch (error) {
        console.log(error);
        return res.status(500).json({ error: error.toString() });
    }
